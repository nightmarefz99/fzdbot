        ...
        try:
            ...
            async with get_db_connection() as db:
                ...
                if current_event["name"] == "NULL":
                    ...
                    await interaction.response.send_message(
                        f"✅ User {interaction.user} has entered a score of {return_score} to {current_event['name']}"
                    )  # , ephemeral=True)
                    logger.info(
                        "User %s entered score=%s for event=%s",
                        interaction.user,
                        score,
                        current_event["name"],
                    )
        ...
        try:
            ...
            async with get_db_connection() as db:
                ...
                if current_event["name"] == "NULL":
                    ...
                    return_score = await submit_score(db, user_data)  # interaction.user
                    await interaction.response.send_message(
                        f"✅ User {interaction.user} has entered rank {rank} → {return_score} points have been added to {current_event['name']}"
                    )  # , ephemeral=True)
                    logger.info(
                        "User %s entered rank=%s (%s points) for event=%s",
                        interaction.user,
                        rank,
                        return_score,
                        current_event["name"],
